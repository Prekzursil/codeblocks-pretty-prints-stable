...
function loadTargets() {
  ...
  if (!file) {
    throw new Error(
      'Set DASHBOARD_TARGETS_JSON or DASHBOARD_TARGETS_FILE before running dashboard verification.',
    );
  }
...
async function verifyTargets(page, outDir, targets) {
  ...
  for (const target of targets) {
    ...
    if (target.expectedText && !text.includes(target.expectedText)) {
      throw new Error(
        'Dashboard ' + target.name + ' did not contain expected text: ' + target.expectedText,
      );
    }
    results.push({
      name: target.name,
