...
def collect_notice_inventory(
    root: str | Path, manifest: Mapping[str, object] | None = None
) -> list[NoticeEntry]:
    ...
    entries = [
        ...
        for category in [
            notice_category_from_name(file_path.name, categories, patterns)
        ]
...
def render_notice_inventory(entries: Sequence[NoticeEntry]) -> str:
    body = [f"- {entry.path} ({entry.category})" for entry in entries] or [
        "- None found"
    ]
