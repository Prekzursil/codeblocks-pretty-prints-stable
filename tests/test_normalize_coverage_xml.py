...
class NormalizeCoverageXmlTests(unittest.TestCase):
    def test_normalize_coverage_xml_paths_prefixes_repo_relative_script_paths(
        self,
    ) -> None:
    ...
    def test_normalize_coverage_xml_paths_is_noop_when_paths_are_already_repo_relative(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            coverage_xml = Path(tempdir) / "coverage.xml"
    ...
    def test_normalize_coverage_xml_paths_ignores_classes_without_filenames_and_main_runs(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            coverage_xml = Path(tempdir) / "coverage.xml"
