...
class PackagingContractTests(unittest.TestCase):
    def test_materialize_profile_seed_writes_normalized_bundle_and_overlay_map(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn(
                "CodeBlocks Stable Toolchain Edition",
    ...
    def test_validate_release_inputs_requires_materialized_profile_seed_and_overlay_contract(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = Path(tempdir)
    ...
    def test_installer_script_uses_safe_delete_guard(self) -> None:
        ...
        self.assertIn(
            "CanSafelyRemoveLegacyInstall(DetectedLegacyInstallPath, ExpandConstant('{app}'))",
            script,
        )
        self.assertIn(
